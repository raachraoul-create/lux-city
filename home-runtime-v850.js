(()=>{let q=setInterval(()=>{let W=window.LuxWorld,H=window.LuxHomes520,I=window.LuxHomeInterior850||window.LuxHomeInterior840,G=window.LuxGameplay632||window.LuxGameplay631;if(!W?.player||!W?.camera||!H?.enter||!H?.leave||!I?.root||!G)return;clearInterval(q);init(W,H,I,G)},70);
function init(W,H,I,G){
 ...
 window.LuxHomeRuntime850={forceInside,hub,get inside(){return!!H.inside}}
}})();