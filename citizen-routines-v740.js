(()=>{let q=setInterval(()=>{if(!window.LuxWorld?.npcs?.length||!window.LuxLife||!window.LuxSystems330?.doors)return;clearInterval(q);const W=LuxWorld,D=LuxSystems330.doors,residents=()=>W.npcs.filter(p=>!p.userData?.customer500&&!p.userData?.customer510&&!p.userData?.customer740&&!p.userData?.routineExcluded);
...
let current='';function update(){let h=(+LuxLife.state.hour||0)+(+LuxLife.state.minute||0)/60,ph=phase(h);if(ph!==current){current=ph;residents().forEach((p,i)=>apply(p,i,ph))}for(let [i,p] of residents().entries()){let own=p.userData.dailyPhase730||ph;if(own==='home'){p.visible=false;p.userData.insideHome730=true}else{p.userData.insideHome730=false;if(own==='work'){let job=p.userData.citizenJob||'',id=jobMap[job];if(id&&window.LuxOpeningHours500&&!LuxOpeningHours500.isStaffed(id))p.visible=false}}}}
setInterval(update,600);update();W.registerTick(()=>{let h=(+LuxLife.state.hour||0)+(+LuxLife.state.minute||0)/60,ph=phase(h);if(ph!==current)update();for(let p of residents()){if(p.userData.insideHome730)p.visible=false}});
window.LuxCitizenRoutines730=window.LuxCitizenRoutines740={phase,routeFor,get current(){return current}}},520)})();