...
window.LuxMobileControlContract850=Object.freeze({left:'a',right:'d',forward:'w',back:'s',interaction:'e',phone:'touch',map:'touch',locked:true});
...
function setKey(k,on){if(on&&!active.has(k)){active.add(k);down(k)}if(!on&&active.has(k)){active.delete(k);up(k)}}
function tapKey(k){down(k);setTimeout(()=>up(k),90);pulse()}
function build(){
 let css=document.createElement('style');css.textContent=`
 #mobileCtl850{position:fixed;inset:0;z-index:63;pointer-events:none;user-select:none;-webkit-user-select:none}
 #joy850{position:absolute;left:max(14px,env(safe-area-inset-left));bottom:max(14px,env(safe-area-inset-bottom));width:116px;height:116px;border-radius:50%;border:2px solid #ffffff99;background:#0b1219a8;box-shadow:0 7px 24px #0008;pointer-events:auto;touch-action:none}
 ...
 @media(orientation:portrait){#joy850{width:122px;height:122px;bottom:20px}#knob850{left:35px;top:35px;width:52px;height:52px}#look850{left:42%;right:82px;top:64px;bottom:98px}.mobileAction850{width:58px;height:58px}#phoneTouch850{bottom:83px}#mapTouch850{bottom:148px}}
 @media(max-height:430px) and (orientation:landscape){#joy850{width:104px;height:104px;bottom:8px}#knob850{left:29px;top:29px;width:46px;height:46px}.mobileAction850{width:54px;height:54px;right:9px}#interact850{bottom:8px}#phoneTouch850{bottom:68px}#mapTouch850{bottom:128px}#look850{top:48px;bottom:68px;right:72px}}
 ...
 look.addEventListener('pointermove',e=>{if(e.pointerId!==lid)return;e.preventDefault();let dx=e.clientX-lx,dy=e.clientY-ly;lx=e.clientX;ly=e.clientY;let ev=new MouseEvent('mousemove',{bubbles:true});try{Object.defineProperty(ev,'movementX',{value:dx*1.45});Object.defineProperty(ev,'movementY',{value:dy*1.45})}catch{}dispatchEvent(ev)});
 ...
 window.LuxMobileUI850={root:r,reset,tapKey}
}
build()})();