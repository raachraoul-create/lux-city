...
function init(){
 ...
 function building(d){
  ...
  let hinge=new T.Group();hinge.position.set(-1.45,0,l/2+.13);let door=add(new T.BoxGeometry(2.9,3.65,.18),M(0x4c3428,.6,.05),1.45,1.825,0,hinge);let glassDoor=add(new T.BoxGeometry(1.85,2.35,.035),glass,1.45,2.05,-.105,hinge);let handle=add(new T.BoxGeometry(.08,.55,.09),dark,2.55,1.82,-.14,hinge);g.add(hinge);doorMeshes[d.id]={hinge,door,open:false};
  add(new T.BoxGeometry(5.4,.2,2.35),M(0x4c5154,.5,.3),0,3.35,l/2+1.0,g);for(let x of[-2.35,2.35])add(new T.CylinderGeometry(.055,.07,3.5,8),dark,x,1.75,l/2+2.05,g);
  ...
  for(let zz=-l/2;zz<=l/2;zz+=2)W.obstacles.push({x:d.x-w/2,z:d.z+zz,r:1.05},{x:d.x+w/2,z:d.z+zz,r:1.05});for(let xx=-w/2;xx<=w/2;xx+=2)W.obstacles.push({x:d.x+xx,z:d.z-l/2,r:1.05});for(let xx=-w/2;xx<=w/2;xx+=2)if(Math.abs(xx)>2.65)W.obstacles.push({x:d.x+xx,z:d.z+l/2,r:1.05});
