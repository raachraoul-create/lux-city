...
function init(){
 ...
 W.registerTick(dt=>{for(let v of emergency){let next=v.position.z+v.userData.speed*dt;let blocked=(W.cars||[]).some(o=>Math.abs(v.position.x-o.position.x)<3&&Math.abs(next-o.position.z)<5)||emergency.some(o=>o!==v&&Math.abs(v.position.x-o.position.x)<3&&Math.abs(next-o.position.z)<5);if(!blocked)v.position.z=next;if(v.position.z>v.userData.baseZ+v.userData.range)v.position.z=v.userData.baseZ-v.userData.range}});
