...
function init(){
 ...
 function emergency(type,x,z){
  ...
  let bar=add(new T.BoxGeometry(1.35,.16,.35),M(isFire?0x2875d4:0x2a76d2,.2,.2),0,h+.75,0,g);g.position.set(x,0,z);g.userData={speed:isFire?5.2:6.2,baseX:x,baseZ:z,range:42,kind:type};S.add(g);return g
 }
 const emergency=[emergency("police",120,-218),emergency("police",126,-190),emergency("ambulance",215,-60),emergency("ambulance",-205,-75),emergency("fire",215,55),emergency("fire",209,25)];
 W.registerTick(dt=>{for(let v of emergency){if(v.userData.target){let dx=v.userData.target.x-v.position.x,dz=v.userData.target.z-v.position.z,dd=Math.hypot(dx,dz);if(dd<4)v.userData.target=null;else{v.position.x+=dx/dd*v.userData.speed*dt;v.position.z+=dz/dd*v.userData.speed*dt;continue}}let next=v.position.z+v.userData.speed*dt;let blocked=(W.cars||[]).some(o=>Math.abs(v.position.x-o.position.x)<3&&Math.abs(next-o.position.z)<5)||emergency.some(o=>o!==v&&Math.abs(v.position.x-o.position.x)<3&&Math.abs(next-o.position.z)<5);if(!blocked)v.position.z=next;if(v.position.z>v.userData.baseZ+v.userData.range)v.position.z=v.userData.baseZ-v.userData.range}});
 ...
 window.LuxCityServices={state,buildings:defs,emergency,dispatch:(x,z,type="ambulance")=>{let v=emergency.find(q=>q.userData.kind===type&&!q.userData.target)||emergency.find(q=>q.userData.kind===type)||emergency[0];if(!v)return false;v.userData.target={x,z};return true},returnToBase:(type)=>{let v=emergency.find(q=>q.userData.kind===type);if(!v)return false;v.userData.target={x:v.userData.baseX,z:v.userData.baseZ};return true},open:ui};
}