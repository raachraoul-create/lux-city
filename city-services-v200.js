...
function init(){
 ...
 function emergency(type,x,z){
  ...
  let bar=add(new T.BoxGeometry(1.35,.16,.35),M(isFire?0x2875d4:0x2a76d2,.2,.2),0,h+.75,0,g);g.position.set(x,0,z);g.userData={speed:isFire?5.2:6.2,baseZ:z,range:42,kind:type};S.add(g);return g
 }
 ...
 window.LuxCityServices={state,buildings:defs,emergency,dispatch:(x,z,type="ambulance")=>{let v=emergency.find(q=>q.userData.kind===type)||emergency[0];if(!v)return false;v.userData.target={x,z};return true},open:ui};
}